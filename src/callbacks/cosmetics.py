...
from src.constants.styles import TRACE_COLOURS, graph_background_colours, APP_THEMES
...
def callback_wrapper(app, chart_default, chart_dark) -> None:
    # Allow instant theme switching in the browser without a round-trip to the server.
    clientside_callback(
    ...
    # Create list of patches matching number of outputs
        ...
        if is_dark:
            for figure in patched_figures:
                figure["layout"]["template"] = pio.templates[chart_default]
                figure["data"][0]["line"]["color"] = TRACE_COLOURS["default"]
        else:
            for figure in patched_figures:
                figure["layout"]["template"] = pio.templates[chart_dark]
                figure["data"][0]["line"]["color"] = TRACE_COLOURS["dark"]
    ...
    def update_trace_colors(is_dark):
        ...
        patch_function_figure["layout"]["template"] = (
            pio.templates[chart_default]
            if is_dark
            else pio.templates[chart_dark]
        )
        ...
        if is_dark:
            for figure in patched_derivative_figures:
                figure["layout"]["template"] = pio.templates[chart_default]
                figure["data"][0]["line"]["color"] = TRACE_COLOURS["default"][i]
                i += 1
        else:
            for figure in patched_derivative_figures:
                figure["layout"]["template"] = pio.templates[chart_dark]
                figure["data"][0]["line"]["color"] = TRACE_COLOURS["dark"][i]
                i += 1
    ...
    def update_slider_background(is_dark):
        ...
        bg_color = (
            GRAPH_BACKGROUND_COLOURS["default"]
            if is_dark
            else GRAPH_BACKGROUND_COLOURS["dark"]
        )
    ...
    def update_controls_background(is_dark):
        ...
        if is_dark:
            bg_color = GRAPH_BACKGROUND_COLOURS["default"]
        else:
            bg_color = GRAPH_BACKGROUND_COLOURS["dark"]
