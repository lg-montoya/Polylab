...
from src.constants.ui_defaults import slider_default
from .factory import modal_callback


# Create x values, avoiding x = 0 to prevent division by zero
x_negative = np.linspace(-slider_default["max"], -0.001, 200)
x_positive = np.linspace(0.001, slider_default["max"], 200)
